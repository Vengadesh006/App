...
export const Nav = ({ showSidebar, setShowNavbar, showNavbar }) => {
  return (
    <div
      className={`fixed top-0 left-0 h-full w-64 bg-[#1B1212] px-3 py-4 text-white z-30 
        flex flex-col items-center justify-around
        transform transition-transform duration-300 ease-in-out 
        ${showNavbar ? "translate-x-0" : "-translate-x-full"} 
        sm:relative sm:translate-x-0 sm:w-[70px]`}
    >
      {/* Logo */}
      <div>
        <img
      ...
      {/* Menu */}
      <ul className="flex flex-col items-center gap-10 mb-10">
        ...
        <li className="flex flex-col items-center cursor-pointer">
          <RiEditBoxLine className="text-4xl" />
